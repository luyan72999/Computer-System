...
int main() {
  ...
  // Queue(allotment, quantum, level, vector)
	...
	Process* p3 = new Process(1004,60);
	process_list.push_back(p3);	
	Process* p4 = new Process(1005,100);
	process_list.push_back(p4);	
	Process* p5 = new Process(1006,150);
	...
	// print the completion and response time of all processes
	...
	int total_r = 0;
	for (int i = 0; i < process_list.size(); i++) {
		total_c += process_list[i]->done_time;
		total_r += process_list[i]->first_response;
	}
	float avg_c = (float)total_c / (float)process_list.size();
	float avg_r = (float)total_r / (float)process_list.size();

	cout << "Average completion time: " + to_string(avg_c) << "\n";
	...
	cout << "Average response time: " + to_string(avg_r) << "\n";
